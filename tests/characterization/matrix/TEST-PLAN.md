...
|---|---|---|
| **Latency / loss / jitter** | Every spec varies *bandwidth only*. `Shape` deliberately omitted `delay_ms`/`loss_pct` ("steady degradation out of scope"). But real links are latency+loss+jitter, which dominate LL-HLS/DASH (TTFB, partial-segment delivery, blocking reload). So §7's bands were "acceptable bandwidth **on a clean link**" — not real-world. | **CLOSED by #826.** `proxy.shape` now carries `delay_ms` / `loss_pct` / `jitter_ms` + `loss_correlation_pct` / `jitter_correlation_pct` (bursty loss, real jitter), plus named link profiles (`clean`/`home`/`mobile-good`/`mobile-poor` + the Apple NLC preset set) as an object-valued `proxy.shape` axis. Delay is one-way (RTT ≈ delay). Re-run a §7 bandwidth spec under `clean`/`home`/`mobile-poor` to show how far the bands shift. Follow-ons: asymmetric `uplink_rate_mbps` (faithful nlc-dsl/3g/edge) and a DNS-delay mechanism (`nlc-high-latency-dns`, not netem-expressible). |
