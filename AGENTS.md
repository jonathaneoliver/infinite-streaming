...
### Encoding Pipeline
- `/generate_abr/create_abr_ladder.sh` (main pipeline)
- `/generate_abr/create_hls_manifests.py`
- `/generate_abr/convert_to_segmentlist.py`

### HLS/DASH Continuous Processes
Go-live manages all live generation and updates internally (no external Python processes).

## Language & Stack
- **Go** (runtime services)
- **nginx** (routing + static UI)
- **Bash/Python helpers** (encoding pipeline only)
- **Dependencies**: nginx, ffmpeg, shaka packager

## Code Style Guidelines
- **Go**: standard `gofmt`
- **Shell**: POSIX-friendly where possible
- **JS/CSS/HTML**: keep UI changes explicit and readable
