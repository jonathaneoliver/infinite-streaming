...
- **Network log (HAR)**: every request the proxy serves to the player is captured with full timing (DNS / connect / TLS / TTFB / wait / transfer), method, URL, status, request kind, fault metadata (`faulted`, `fault_type`, `fault_action`, `fault_category`), and the kernel‑measured **`delivery_rate_mbps`** (`tcpi_delivery_rate`, #850) — the honest per‑request wire rate under tc shaping.
   - The waterfall's **Mbps column shows `delivery_rate_mbps` when present**, falling back to the implied `bytes_out/transfer_ms` figure only for rows that predate the field (old archives, non‑Linux dev proxy). The implied figure times only the write into the socket send buffer and over‑reads up to ~4900× on sub‑buffer transfers; the row tooltip shows both rates side by side. Kernel‑rate caveat: connection‑level, so below ~64 KB it reflects the socket's recent history, not that one request.
   - The **bandwidth chart overlays "Delivery rate (kernel)" dots** — one emerald dot per segment request ≥ 256 KB (the calibrated honesty floor, server‑behavior standard §1.14) — toggled via the same legend‑chip mechanism as the iOS per‑segment fetch dots. Active session only in compare mode.
