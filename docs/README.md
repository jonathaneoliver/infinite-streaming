...
| [CONTENT_FORMAT.md](CONTENT_FORMAT.md) | The content contract for anything placed in `/media/dynamic_content/` — directory naming and what each part drives, required files, partial-segment info for LL. Read before bringing your own encodes |
...
- **`generate_abr/`** — encoding-pipeline docs (ABR ladder, packager
  ...
  fallback, not the primary — content is normally produced by
  [infinite-streaming-encoder](https://github.com/jonathaneoliver/infinite-streaming-encoder).
  Start at [`generate_abr/README.md`](../generate_abr/README.md).
